            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    Eigen::Array3f Le(images[i][index + 0], images[i][index + 1],
                                      images[i][index + 2]);
                    // my code!!!!!
                    sumWeight = CalcArea(x, y, width, height);
                    for (int l = 0; l <= SHOrder; l++) {
                        for (int m = -l; m <= l; m++) {
                            auto dir3d = Eigen::Vector3d{static_cast<double>(dir.x()),static_cast<double>(dir.y()),static_cast<double>(dir.z()) };
                            double sh = sh::EvalSH(l, m, dir3d.normalized());
                            auto fsh = static_cast<float>(sh);
                            SHCoeffiecents[sh::GetIndex(l, m)] += Le * fsh * sumWeight;
                        }
                    }
                }
            }
