	...
	bindMaterialParameters() {
		...
		for (let k in this.material.uniforms) {
			...
			if (this.material.uniforms[k].type == 'matrix4fv') {
				...
				gl.uniform1i(this.shader.program.uniforms[k], textureNum);
				textureNum += 1;
			} else if (this.material.uniforms[k].type == 'uPrecomputeLR') {
				let precomputeLRGB = getMat3ValueFromRGB(precomputeL[guiParams.envmapId]);
				gl.uniformMatrix3fv(
					this.shader.program.uniforms[k],
					false,
					precomputeLRGB[0]);
			}else if (this.material.uniforms[k].type == 'uPrecomputeLG') {
				let precomputeLRGB = getMat3ValueFromRGB(precomputeL[guiParams.envmapId]);
				gl.uniformMatrix3fv(
					this.shader.program.uniforms[k],
					false,
					precomputeLRGB[1]);
			}else if (this.material.uniforms[k].type == 'uPrecomputeLB') {
				let precomputeLRGB = getMat3ValueFromRGB(precomputeL[guiParams.envmapId]);
				gl.uniformMatrix3fv(
					this.shader.program.uniforms[k],
					false,
